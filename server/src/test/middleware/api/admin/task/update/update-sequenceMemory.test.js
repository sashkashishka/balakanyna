...
import { sequenceMemoryTask } from '../../fixtures/task.js';
import { assertCommonTaskProps } from '../utils.js';

const hash = '88888888';

describe('[api] task update sequenceMemory', () => {
  test('should return 400 if config is invalid', async (t) => {
    ...
    const { request } = await getTestServer({
      ...
      async seed(db, config) {
        ...
        dbTasks = await seedTasks(db, [sequenceMemoryTask]);
      },
    });
  ...
  test('should return 200 and update task', async (t) => {
    ...
    const { request } = await getTestServer({
      ...
      async seed(db, config) {
        await seedAdmins(db, [admin], config.salt.password);
        dbTasks = await seedTasks(db, [{ ...sequenceMemoryTask, hash }]);
      },
    });
    ...
    assert.equal(body.config.width, payload.config.width);
    assert.equal(body.config.height, payload.config.height);
  });
});